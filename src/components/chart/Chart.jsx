...
const Chart = (props) => {
  ...
  const filterTypes = () => {
    if (state.allTypes) { 
      ...
      console.log(fail)
      return (
        pass.sort((a, b) => parseInt(b.probability[2]) - parseInt(a.probability[2]))
          .concat(fail.sort((a, b) => parseInt(b.probability[2]) - parseInt(a.probability[2]))))
    } else {
      return (
        types.filter((e) => e.count > 1000).sort((a, b) => parseInt(b.probability[2]) - parseInt(a.probability[2])))
    }
  } 
              ...
              <Card className={classes.root} onMouseEnter={null} onMouseLeave={() => {}} >
                <CardContent className={classes.content}>
                  <Grid container direction='row' alignItems='center' spacing={2}>
                    ...
                    <Grid item xs={8} >
                      <ChartProgressBar
                        bad={ item.probability[0] }
                        normal={ item.probability[1] }
                        good={ item.probability[2] }
                        hover={ item.hover } />
